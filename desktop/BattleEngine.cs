...
namespace AABattle {
 ...
 public class TrainerBattleState {
  ...
  public TrainerBattleState Copy(){var s=new TrainerBattleState();s.UsedForms=new HashSet<string>(UsedForms);s.UsedOrders=new HashSet<string>(UsedOrders);s.UsedPotentials=new HashSet<string>(UsedPotentials);return s;}
 }
 public class BattleCommand {
  public Move Move;public Fighter SwitchIn;public string Form="",Order="";public string[] PreInputs=new string[0];public Fighter[] Party=new Fighter[0];
 ...
 public static class Engine {
  ...
  public static TurnResult Resolve(Database db,Fighter[] input,BattleCommand[] commands,Rules original,TrainerBattleState[] trainerInput,Func<double> rng){
   ...
   for(int i=0;i<2;i++){
    ...
    if(order.Length>0)RuntimeTriggers.DirectiveReceived(f[i],(c.Party??new Fighter[0]).Where(x=>x!=null&&x.Data.name!=f[i].Data.name).Concat(new[]{f[i]}),order,result.Log.Add);
   }
  ...
  public static string Test(Database db){
   ...
   var tap=new Move{name="약한공격",category=m.category,types=m.types,power=1,accuracy=100,priority=7,attack=m.attack,defense=m.defense};var receiver=TriggerFighter(b,"수신자","반격",TriggerStructures.ReceivedLabel);var received=Resolve(db,new[]{a,receiver},new[]{tap,protect},new Rules(),()=>0);Check(received.Log.Any(x=>x.Contains("『반격』")&&x.Contains(TriggerStructures.ReceivedLabel)),"received once after move");var switcher=TriggerFighter(a,"교대자","선회",TriggerStructures.SwitchLabel);var reserveForTest=new Fighter(db.pokemon.First(p=>p.name=="원시 가이오가"));var switched=Resolve(db,new[]{switcher,b},new[]{new BattleCommand{SwitchIn=reserveForTest},new BattleCommand{Move=protect}},new Rules(),new[]{new TrainerBattleState(),new TrainerBattleState()},()=>0);Check(switched.Log.Any(x=>x.Contains("『선회』")&&x.Contains(TriggerStructures.SwitchLabel)),"voluntary switch trigger");var ender=TriggerFighter(a,"종료자","윤회",TriggerStructures.TurnEndLabel);ender.Status="독";var ended=Resolve(db,new[]{ender,b},new[]{protect,protect},new Rules(),()=>0);int residualAt=ended.Log.FindIndex(x=>x.Contains("독: HP")),triggerAt=ended.Log.FindIndex(x=>x.Contains("『윤회』"));Check(residualAt>=0&&triggerAt>residualAt,"turn end trigger after residual");
