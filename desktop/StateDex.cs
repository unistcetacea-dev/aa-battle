...
namespace AABattle {
 ...
 public static class StateDex {
  public static List<StateDexEntry> Entries(){
   ...
   Add(result,"포텐셜 수치 효과","다른 종족치 복사","‘특방을 특공과 같게 한다’와 ‘특방을 특공과 동랭크까지 올린다’는 모두 현재 대소 관계를 확인하지 않고 특공의 실제 종족치 숫자를 특방에 그대로 복사합니다.");
   ...
   Add(result,"포텐셜 트리거","X / 상시","트리거 X는 별도의 발동 시점 없이 효과가 계속 적용되는 상시 효과입니다.");
   Add(result,"포텐셜 트리거","장소에 나올 때","‘필드에 나올 때’, ‘장소에 나왔을 때’ 등의 표현을 ‘장소에 나올 때’로 통일합니다. 선발, 통상교대, 죽어내밀기 등으로 등장하면 성립하지만 강제교대로 끌려 나온 경우에는 성립하지 않습니다.");
   Add(result,"포텐셜 트리거","상대를 쓰러뜨렸을 때","이 포텐셜을 가진 포켓몬이 사용한 기술의 직접 대미지로 상대 포켓몬의 HP를 0으로 만들었을 때 성립합니다. 독·화상·날씨·설치물·반동·포텐셜 대미지로 쓰러진 경우에는 성립하지 않습니다. ‘쓰러트렸을 때’도 같은 트리거로 통일합니다.");
   Add(result,"포텐셜 트리거","PT에 참여하고 있을 때","옵저버 트리거입니다. 포켓몬이 HP가 남은 대기 멤버일 때만 성립하며, 장소에 나와 있거나 빈사 상태라면 성립하지 않습니다. 대기 중에도 고유 포텐셜처럼 파티 전체에 효과를 줄 수 있습니다. ‘참가하고 있을 때’, ‘참가하고 있는 한’, ‘참여중일 때’를 같은 표현으로 통일합니다.");
   Add(result,"포텐셜 트리거","T 종료 시","슬립 대미지, 드레인, 회복, 턴 종료 상태 해제를 모두 처리한 뒤 성립합니다. 같은 시점의 포켓몬 효과는 현재 속도가 빠른 쪽부터 처리합니다. ‘T종료시’, ‘턴 종료시’, ‘매T종료 시에’를 같은 표현으로 통일합니다.");
   Add(result,"포텐셜 트리거","장소에 있는 한","현재 장소에 있고 HP가 남아 있는 동안 X와 같은 상시 효과로 성립합니다. 장소에 나온 직후 시작하며 교대하거나 빈사가 되는 즉시 끝납니다. ‘필드에 있는 한’, ‘필드에 있을 때’를 같은 표현으로 통일합니다.");
   Add(result,"포텐셜 트리거","아군과 임의교대할 때","통상교대, 『돌아와！』, 유턴 계열, 포텐셜에 의한 교대처럼 교대 주체가 선택할 수 있는 모든 교대에서 성립합니다. 강제교대만 제외합니다. 통상교대는 행동 대신 선택하는 교대와 『돌아와！』를 뜻합니다. 수식 없는 ‘아군과 교대할 때’도 이 표현으로 통일합니다.");
   Add(result,"포텐셜 트리거","상대의 공격을 받았을 때","상대의 공격 기술 한 번이 모두 끝난 뒤, 포켓몬 본체가 직접 대미지를 받았고 살아 있을 때 한 번 성립합니다. 연속기는 타격마다 판정하지 않습니다. 회피·무효화·대타가 받은 대미지에는 성립하지 않습니다. ‘공격을 받으면’을 같은 표현으로 통일합니다.");
   Add(result,"포텐셜 트리거","장소를 떠날 때","강제교대를 제외한 교대와 빈사로 장소를 떠날 때 성립합니다. 빈사 퇴장도 성립하지만 강제교대로 끌려 나가는 경우에는 성립하지 않습니다.");
   Add(result,"포텐셜 트리거","등장 방법 지정","선발, 죽어내밀기, 임의교대, 선발 또는 죽어내밀기를 각각 선택합니다. 임의교대에는 통상교대·돌아와·유턴 계열·포텐셜 교대가 포함되며 강제교대는 제외합니다.");
   Add(result,"포텐셜 트리거","적진에 타입 포켓몬이 있을 때","상대의 현재 타입 중 선택한 타입이 있을 때 성립합니다. 원래 타입, 추가 타입, 일시적으로 변경된 타입을 모두 현재 타입으로 판정합니다.");
   Add(result,"포텐셜 트리거","트레이너 자질 등급","지시·육성·통솔·능력 중 하나와 알파벳 등급을 선택합니다. 부호를 제외한 알파벳이 같은 등급대만 성립하므로 A-, A, A+는 A 조건에 맞고 AA는 맞지 않습니다.");
   Add(result,"포텐셜 트리거","상대의 공격을 받았거나 무효화했을 때","공격 기술 한 번의 처리가 끝난 뒤 본체가 직접 대미지를 받았거나, 타입·특성·도구·방호 포텐셜로 공격을 무효화했을 때 성립합니다. 방어·판별, 회피, 빗나감, 대타는 제외합니다.");
   Add(result,"포텐셜 트리거","「여기다！」일 때","시작 편성이 6체 이상이면 생존 3체 이하, 4~5체면 2체 이하, 3체 이하면 최후의 1체일 때 성립합니다. 적진에 살아 있는 『에이스』 또는 『탐사대장』이 나와 있어도 성립하며, 한번 성립하면 부활로 인원이 늘어도 시합 종료까지 유지합니다.");
   Add(result,"포텐셜 트리거","자신·아군이 지령을 받았을 때","‘자신’은 현재 이 포텐셜을 가진 포켓몬이 지령을 받은 경우입니다. ‘아군’은 파티의 어느 포켓몬이 받아도 성립하는 파티 감시 범위이며, 다른 트리거에서도 자신과 아군을 같은 범위로 합치지 않습니다.");
   Add(result,"포텐셜 트리거","XX가 부여되어 있을 때","선택하거나 입력한 포텐셜이 해당 포켓몬에게 실제로 부여되어 있는 동안 성립합니다. 『계약의 특권』 외의 이름에도 같은 구조를 사용합니다.");
   Add(result,"포텐셜 트리거","상대의 데이터를 해석했을 때","아직 해석하지 않은 상대의 데이터가 처음 공개된 직후 한 번 성립합니다. 이미 해석한 같은 상대는 다시 해석할 수 없습니다.");
   Add(result,"포텐셜 트리거","배틀 시작 시","양쪽 팀과 선발을 확정한 뒤 배틀이 처음 시작되는 시점에 한 번 성립합니다. ‘배틀 개시시’, ‘시합 시작 시’를 같은 구조로 통일합니다.");
   Add(result,"포텐셜 트리거","상대가 장소에 나왔을 때","상대가 선발·교대·죽어내밀기·강제교대 등 어떤 방식으로든 장소에 나오면 성립합니다.");
   Add(result,"포텐셜 트리거","기술을 내보낼 때","실제로 행동할 수 있음이 확정된 뒤 기술 처리 직전에 성립합니다. 전체 기술, 공격기술, 변화기술, 선택한 타입의 기술, 선택한 특정 기술을 각각 구조로 지정할 수 있습니다.");
   Add(result,"포텐셜 트리거","공격이 성공했을 때","공격기술이 명중해 한 번의 기술 처리가 끝났을 때 성립합니다. 변화기술은 명중해도 공격 성공으로 취급하지 않습니다.");
   Add(result,"포텐셜 트리거","대미지를 받았을 때","상대 기술뿐 아니라 반동·날씨·상태이상·설치기·포텐셜 등으로 실제 HP가 줄었을 때 성립합니다. 이 대미지로 빈사가 되어도 성립합니다.");
   Add(result,"배틀 정보","도감 데이터 해석","배틀 중 도감을 사용해 상대를 해석할 수 있습니다. 아스키아트, 레벨, 타입, 특성, 종족치, 기술과 일반 포텐셜을 공개하며 현재 체력, 도구, 전용 포텐셜은 공개하지 않습니다.");
   Add(result,"포텐셜 입력","선행입력","선행 발동 방식으로 저장된 포텐셜은 사용할 수 있을 때 지시창에 선택 항목이 나타납니다. ‘할 수 있다’ 효과는 실제 발동 여부를 묻고 거절하면 소비하지 않습니다. 의미가 불명확한 ‘턴 중 선언 시’ 조건은 제외합니다.");
   Add(result,"특권","고정 특권 템플릿","18타입 『익스펜션』과 『계약의 특권』『주인의 특권』『왕자의 특권』을 선택합니다. 복합 효과는 줄마다 독립된 구조 효과로 저장합니다. 계약은 지정 타입 기술의 대미지를 반감하며, 주인·왕자의 종족치 ‘조금’ 값은 계산 설정에서 바꿉니다.");
   ...
   Add(result,"상태변화","충전","다음 전기 타입 기술의 위력을 2배로 합니다. 해당 기술을 내보내거나 일반적인 턴 종료로 해제되며, 임의교대 직후의 턴 종료 처리는 별도 규칙으로 구분합니다.");
   Add(result,"상태변화","가열","다음 불꽃 타입 기술의 위력을 2배로 합니다. 해당 기술을 내보내거나 일반적인 턴 종료로 해제되며, 임의교대 직후의 턴 종료 처리는 별도 규칙으로 구분합니다.");
   Add(result,"상태변화","냉각","다음 얼음 타입 기술의 위력을 2배로 합니다. 해당 기술을 내보내거나 일반적인 턴 종료로 해제되며, 임의교대 직후의 턴 종료 처리는 별도 규칙으로 구분합니다.");
   Add(result,"상태변화","폭탄","폭탄 상태에서 접촉 기술을 받으면 최대 HP의 1/4만큼 체력을 잃습니다. 명중 후·행동 후 또는 턴 경과로 해제되며 『바톤』으로 인계됩니다.");
   Add(result,"상태변화","미래예지","다음 T의 가장 마지막에 명중판정과 대미지 판정을 합니다. 사용 당시 공격자의 특공을 저장하며, 사용 측의 능력변화·배율변화·포텐셜은 반영하지 않습니다. 방어·매직가드·『피해라！』·『견뎌라！』의 타이밍이 없고 대상이 교대하면 해제됩니다.");
   Add(result,"상태변화","번개구름","미래예지 계열의 예약 공격 상태입니다. 사용 당시 공격자의 공격능력치로 판정하며, 이 프로젝트 규칙에서는 대상이 교대하면 해제됩니다.");
   Add(result,"상태변화","난동부리기","2~3턴 동안 같은 공격을 반복하고, 난동이 끝나면 혼란이 됩니다.");
   Add(result,"상태변화","사슬묶기","대상이 마지막으로 사용한 기술을 4턴 동안 사용할 수 없게 합니다.");
   Add(result,"상태변화","스포트라이트","그 턴이 끝날 때까지 상대의 단일 대상 공격을 지정한 대상에게 유도합니다. 더블배틀 계열에서 사용하는 상태입니다.");
   Add(result,"상태변화","조이기","4~5턴 동안 교대를 제한하고 턴 종료 시 최대 HP의 1/8 피해를 줍니다. 사용자 또는 대상이 장소를 떠나면 해제됩니다.");
   Add(result,"진영 상태",RuleBook.StatusHelp,new[]{"리플렉터·빛의장막","순풍"});
   Add(result,"진영 상태","흙놀이","전체 필드를 5턴 동안 흙놀이 상태로 하여 전기 타입 기술의 위력을 약화합니다. 포켓몬 개인 상태가 아니라 진영·필드 상태입니다.");
  ...
  public static string[] StatusNames(string category){return Entries().Where(x=>x.category==category).Select(x=>x.name).Distinct().ToArray();}
