...
namespace AABattle {
 ...
 static class FlowPreview {
  public static void Run(Database db,string path){
   ...
   using(var start=new StartForm(db)){start.SetProjectForTest(saved);start.SetFileTeamForTest(0,saved.teams[1]);start.Show();Application.DoEvents();Capture(start,path+".start.png");start.Close();}
 ...
 class StartForm:Form {
  Database db;EditorProject project=new EditorProject();FlatButton start,edit,dex;ComboBox[] teamPick=new ComboBox[2];FlatButton[] teamLoad=new FlatButton[2];PixelLabel[] teamInfo=new PixelLabel[2];TeamRecord[] fileTeams=new TeamRecord[2];bool refreshingTeams;
  public StartForm(Database d){db=d;Text="포켓몬 AA 배틀";ClientSize=new Size(1100,740);MinimumSize=new Size(900,640);StartPosition=FormStartPosition.CenterScreen;BackColor=Color.White;DoubleBuffered=true;Font=Theme.UI();
   ...
   for(int i=0;i<2;i++){int side=i;teamPick[i]=Theme.Combo(new string[0],"");teamPick[i].Dock=DockStyle.None;teamPick[i].Size=new Size(330,38);teamPick[i].DropDownStyle=ComboBoxStyle.DropDownList;teamPick[i].SelectedIndexChanged+=(s,e)=>{if(!refreshingTeams)fileTeams[side]=null;RefreshTeamInfo(side);};teamLoad[i]=new FlatButton{Text="파일에서 팀 불러오기",Size=new Size(330,38)};teamLoad[i].Click+=(s,e)=>LoadTeamFile(side);teamInfo[i]=new PixelLabel{Size=new Size(330,42),TextAlign=ContentAlignment.MiddleCenter};Controls.Add(teamPick[i]);Controls.Add(teamLoad[i]);Controls.Add(teamInfo[i]);}
  ...
  internal void SetFileTeamForTest(int side,TeamRecord team){fileTeams[side]=team;refreshingTeams=true;teamPick[side].Items.Clear();teamPick[side].Items.Add(team.ToString());teamPick[side].SelectedIndex=0;refreshingTeams=false;RefreshTeamInfo(side);if(!Object.ReferenceEquals(SelectedTeam(side),team))throw new Exception("Start screen file team selection failed");}
  void RefreshTeamChoices(){var names=(project.teams??new List<TeamRecord>()).Select(x=>x.ToString()).ToArray();refreshingTeams=true;for(int side=0;side<2;side++){int old=teamPick[side].SelectedIndex;teamPick[side].Items.Clear();teamPick[side].Items.AddRange(names);if(names.Length>0)teamPick[side].SelectedIndex=Math.Min(old>=0?old:side,names.Length-1);fileTeams[side]=null;RefreshTeamInfo(side);}refreshingTeams=false;}
  TeamRecord SelectedTeam(int side){if(fileTeams[side]!=null)return fileTeams[side];int i=teamPick[side].SelectedIndex;return project.teams!=null&&i>=0&&i<project.teams.Count?project.teams[i]:null;}
  void RefreshTeamInfo(int side){var team=SelectedTeam(side);teamInfo[side].Text=team!=null?(side+1)+"팀 · "+team.name+" · "+team.members.Count+"체":(side+1)+"팀 · 팀 데이터를 선택하세요";}
  void LoadTeamFile(int side){var dialog=new OpenFileDialog{Filter="AA Battle 데이터 (*.aabdata.json)|*.aabdata.json|JSON (*.json)|*.json"};if(dialog.ShowDialog(this)!=DialogResult.OK)return;try{var loaded=new JavaScriptSerializer{MaxJsonLength=50000000}.Deserialize<EditorProject>(File.ReadAllText(dialog.FileName,Encoding.UTF8));var teams=loaded==null||loaded.teams==null?new List<TeamRecord>():loaded.teams.Where(x=>x!=null).ToList();if(teams.Count==0)throw new Exception("저장된 팀이 없습니다.");TeamRecord chosen=teams.Count==1?teams[0]:ChooseTeam(teams,side);if(chosen==null)return;fileTeams[side]=chosen;refreshingTeams=true;teamPick[side].Items.Clear();teamPick[side].Items.Add(chosen.ToString());teamPick[side].SelectedIndex=0;refreshingTeams=false;RefreshTeamInfo(side);}catch(Exception ex){MessageBox.Show(this,"팀 데이터를 불러올 수 없습니다.\r\n"+ex.Message,"불러오기 오류");}}
  TeamRecord ChooseTeam(List<TeamRecord> teams,int side){using(var form=new Form{Text=(side+1)+"팀 선택",Size=new Size(480,430),StartPosition=FormStartPosition.CenterParent,BackColor=Color.White}){var list=new PixelChoiceList{Dock=DockStyle.Fill};foreach(var team in teams)list.Items.Add(team);list.SelectedIndex=0;var ok=new FlatButton{Text="이 팀 불러오기",Dock=DockStyle.Bottom,Height=50,Primary=true,DialogResult=DialogResult.OK};form.Controls.Add(list);form.Controls.Add(ok);return form.ShowDialog(this)==DialogResult.OK?teams[list.SelectedIndex]:null;}}
  void Arrange(){dex.Location=new Point(ClientSize.Width-270,65);int gap=70,left=(ClientSize.Width-660-gap)/2;teamPick[0].Location=new Point(left,335);teamPick[1].Location=new Point(left+330+gap,335);teamLoad[0].Location=new Point(left,380);teamLoad[1].Location=new Point(left+330+gap,380);teamInfo[0].Location=new Point(left,423);teamInfo[1].Location=new Point(left+330+gap,423);start.Location=new Point((ClientSize.Width-290)/2,500);edit.Location=new Point(start.Left,start.Top+82);}
  ...
  void Begin(){try{var teams=new[]{SelectedTeam(0),SelectedTeam(1)};if(teams.Any(x=>x==null))throw new Exception("1팀과 2팀에서 저장 파일을 불러오거나 팀을 선택하세요.");if(teams.Any(x=>x.trainer==null||x.members==null||x.members.Count<1))throw new Exception("선택한 팀에 트레이너와 포켓몬이 필요합니다.");
 ...
 partial class MainForm {
  ...
  public void LoadTeams(TeamRecord[] teams,List<Fighter>[] all,int[] leads){for(int i=0;i<2;i++){trainers[i]=teams[i].trainer;trainerStates[i].Trainer=trainers[i];initialTeamSizes[i]=all[i].Count;here[i]=false;analyzed[i].Clear();Fighters[i]=all[i][leads[i]];reserves[i]=all[i].Where((x,n)=>n!=leads[i]).ToList();}RuntimeTriggers.BattleStart(trainers,new IEnumerable<Fighter>[]{all[0],all[1]},x=>log+=x+"\r\n");for(int i=0;i<2;i++)EntryTriggers.Enter(Fighters[i],EntryReason.Lead,logLine=>log+=logLine+"\r\n");RuntimeTriggers.OpponentEntered(Fighters[0],Fighters[1],x=>log+=x+"\r\n");RuntimeTriggers.OpponentEntered(Fighters[1],Fighters[0],x=>log+=x+"\r\n");undo.Clear();RefreshView();}
 }
}
