...
namespace AABattle {
 ...
 public static class SpeciesStatRanks {
  ...
  public static int[] RaisedRange(int value){int tier=Tier(value),modifier=Modifier(value,tier);int next=Math.Min(Starts.Length-1,tier+1);if(next==Starts.Length-1)return new[]{200,999};int start=Starts[next],end=Starts[next+1]-1;if(modifier<0)return new[]{start,start};if(modifier>0)return new[]{end-4,end};return new[]{start+1,end-5};}
  ...
  public static void Test(){if(!RaisedRange(90).SequenceEqual(new[]{110,110})||!RaisedRange(100).SequenceEqual(new[]{111,134})||!RaisedRange(105).SequenceEqual(new[]{135,139})||!RaisedRange(135).SequenceEqual(new[]{165,169}))throw new Exception("Species stat rank ranges failed");}
 }
 ...
 partial class DataEditorForm {
  ...
  void RefreshEffectActions(){if(effectKind==null||effectAction==null)return;string[] choices=effectKind.Text=="종족치 변경"?new[]{"가장 높은 종족치 복사","가장 낮은 종족치 복사","1랭크 상승","직접 수치 지정"}:effectKind.Text=="수치 배율"?new[]{"강화","약화"}:effectKind.Text=="최대 HP의 1/N"?new[]{"회복","대미지"}:new[]{"상승","하락"};string current=effectAction.Text;if(effectAction.Items.Cast<object>().Select(x=>x.ToString()).SequenceEqual(choices))return;effectAction.Items.Clear();effectAction.Items.AddRange(choices.Cast<object>().ToArray());effectAction.SelectedItem=choices.Contains(current)?current:choices[0];}
  ...
  void RefreshSpeciesRange(){int min=1,max=999;if(effectAction.Text=="1랭크 상승"&&CurrentSpeciesValue()>0){var range=SpeciesStatRanks.RaisedRange(CurrentSpeciesValue());min=range[0];max=range[1];}effectSpeciesValue.Maximum=max;effectSpeciesValue.Minimum=min;if(effectSpeciesValue.Value<min)effectSpeciesValue.Value=min;if(effectSpeciesValue.Value>max)effectSpeciesValue.Value=max;}
  void RefreshStructuredEffect(){if(effectPreview==null)return;bool scalar=effectKind.Text=="수치 배율",fraction=effectKind.Text=="최대 HP의 1/N",rank=effectKind.Text=="능력 랭크 변화",species=effectKind.Text=="종족치 변경";effectRate.Enabled=scalar;effectDenominator.Enabled=fraction||rank;effectSpeciesValue.Enabled=species&&(effectAction.Text=="1랭크 상승"||effectAction.Text=="직접 수치 지정");effectCanFaint.Enabled=fraction&&effectAction.Text=="대미지";if(species){RefreshSpeciesRange();string target=effectTarget.Text+" 종족치";if(effectAction.Text=="가장 높은 종족치 복사")effectPreview.Text=target+"를 가장 높은 종족치로 한다.";else if(effectAction.Text=="가장 낮은 종족치 복사")effectPreview.Text=target+"를 가장 낮은 종족치로 한다.";else if(effectAction.Text=="1랭크 상승"&&CurrentSpeciesValue()<=0)effectPreview.Text="포켓몬과 대상 종족치를 먼저 설정하세요.";else if(effectAction.Text=="1랭크 상승")effectPreview.Text=target+"를 「"+(int)effectSpeciesValue.Value+"」로 하여 1랭크 올린다.";else effectPreview.Text=target+"를 「"+(int)effectSpeciesValue.Value+"」로 한다.";}else if(scalar){effectPreview.Text=EffectNormalizer.Canonical(effectTarget.Text,effectAction.Text=="약화"?"약화":"강화",(double)effectRate.Value);}else if(fraction){string target=new[]{"자신","상대","아군 1체","아군 전체"}.Contains(effectTarget.Text)?effectTarget.Text:"자신";int n=(int)effectDenominator.Value;effectPreview.Text=effectAction.Text=="대미지"?target+"에게 최대 HP의 1/"+n+" 대미지를 준다."+(effectCanFaint.Checked?"":" 이 대미지로는 빈사로 할 수 없다."):target+"의 체력을 최대 HP의 1/"+n+" 회복한다.";}else if(rank){if(effectDenominator.Value>3)effectDenominator.Value=3;effectPreview.Text=EffectNormalizer.CanonicalRank(effectTarget.Text,(effectAction.Text=="하락"?-1:1)*(int)effectDenominator.Value);}else effectPreview.Text="";effectKind.Invalidate();effectAction.Invalidate();}
