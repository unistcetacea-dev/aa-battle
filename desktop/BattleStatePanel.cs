...
namespace AABattle {
 ...
 public static string Build(Database db,Fighter[] fighters,Rules rules,TurnSelection[] selections,TrainerRecord[] trainers,TrainerBattleState[] trainerStates,IEnumerable<Fighter>[] reserves=null){
  ...
  static void FighterBlock(StringBuilder text,Database db,Fighter a,Fighter b,Rules r,TurnSelection selected,int side,TrainerRecord trainer){
   ...
   var active=RuntimeTriggers.Field(a,b,trainer).Select(x=>"『"+x.name+"』").ToArray();text.Append("상시 트리거 성립: ").AppendLine(active.Length==0?"없음":string.Join(" ",active));text.Append("등록 포텐셜(판정 대기): ");var potentials=(a.Data.potentials??new Potential[0]).Where(x=>x!=null).Select(x=>"『"+x.name+"』").ToArray();text.AppendLine(potentials.Length==0?"없음":string.Join(" ",potentials));
