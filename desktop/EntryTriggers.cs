...
namespace AABattle {
 ...
 public static class RuntimeTriggers {
  ...
  public static void BattleStart(TrainerRecord[] trainers,IEnumerable<Fighter>[] parties,Action<string> log){for(int side=0;side<2;side++){var trainer=trainers!=null&&side<trainers.Length?trainers[side]:null;if(trainer!=null)foreach(var p in trainer.potentials??new List<PotentialRecord>())if(TriggerStructures.NormalizeLines(p.trigger).Split(new[]{'\r','\n'},StringSplitOptions.RemoveEmptyEntries).Contains(TriggerStructures.BattleStartLabel))log("TRAINER "+(side+1)+" 『"+p.name+"』 "+TriggerStructures.BattleStartLabel+" 트리거 충족 · 효과 판정 대기: "+p.effect);foreach(var owner in parties!=null&&side<parties.Length?parties[side]:Enumerable.Empty<Fighter>())if(owner!=null&&owner.HP>0&&TriggerStructures.Has(owner,TriggerStructures.BattleStartLabel))TriggerStructures.Fire(owner,TriggerStructures.BattleStartLabel,"배틀이 시작됐다.",log);}}
