...
namespace AABattle {
 ...
 public static class EffectImplementationDex {
  ...
  static string Prepared(string family){
   switch(family){
    case "종족치 변경":return "대상 종족치에 직접 수치를 지정하거나 가장 높은/낮은 종족치의 실제 숫자를 복사합니다. 1·2랭크 상승은 기존의 -, 무표기, +를 유지한 도착 등급 범위에서 값을 고릅니다. 지정 등급까지 상승·하락은 기준을 이미 넘은 값을 유지하고, 지정 등급으로 변경은 현재 값과 관계없이 바꿉니다. 동률 비교는 후보 중 무작위입니다. 포텐셜 실행기 연결은 아직 준비 단계입니다.";
