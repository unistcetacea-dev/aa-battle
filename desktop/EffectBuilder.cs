...
namespace AABattle {
 public static class EffectNormalizer {
  ...
  public static string CanonicalRank(string target,int amount){target=PotentialParts.Clean(target);amount=Math.Max(1,Math.Min(3,amount));return target+SubjectParticle(target)+" "+(amount==3?"매우 크게 ":amount==2?"크게 ":"")+"오른다.";}
 ...
 public static class EffectStructures {
  ...
  public static bool IsRankUp(string text){return Regex.IsMatch(PotentialParts.Clean(EffectNormalizer.Normalize(text)),@"^.+?(?:이|가)\s*(?:매우 크게 |크게 )?오른다$");}
  ...
  public static List<PotentialPart> Collapse(IEnumerable<PotentialPart> source){var input=source.Where(x=>!x.excludedFromImplementation).ToList();var result=new List<PotentialPart>();var scalar=input.Where(x=>!x.protectedTemplate&&IsScalar(x.text)).ToList();var fraction=input.Where(x=>!x.protectedTemplate&&TryFractionOnly(x.text)).ToList();var rank=input.Where(x=>!x.protectedTemplate&&IsRankUp(x.text)).ToList();var grouped=new HashSet<PotentialPart>(scalar.Concat(fraction).Concat(rank));result.AddRange(input.Where(x=>!grouped.Contains(x)));if(scalar.Count>0)result.Add(Template("scalar","수치 배율 — 대상 / 강화·약화 / 배율을 선택",scalar));if(fraction.Count>0)result.Add(Template("fraction","최대 HP의 1/N — 대상 / 회복·대미지 / 빈사 여부를 선택",fraction));if(rank.Count>0)result.Add(Template("rank-up","능력 랭크 상승 — 대상 / +1·+2·+3을 선택",rank));return result.OrderBy(x=>x.text).ToList();}
 ...
 partial class DataEditorForm {
  ...
  Control BuildInlineEffectBuilder(){
   ...
   effectKind=Theme.Combo(new[]{"선택 원문 그대로","수치 배율","최대 HP의 1/N","능력 랭크 상승"},"선택 원문 그대로");effectKind.DrawMode=DrawMode.Normal;
