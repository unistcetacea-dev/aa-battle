...
namespace AABattle {
 ...
 partial class MainForm {
  ...
  public void LoadTeams(TeamRecord[] teams,List<Fighter>[] all,int[] leads){for(int i=0;i<2;i++){trainers[i]=teams[i].trainer;initialTeamSizes[i]=all[i].Count;here[i]=false;analyzed[i].Clear();Fighters[i]=all[i][leads[i]];reserves[i]=all[i].Where((x,n)=>n!=leads[i]).ToList();}RuntimeTriggers.BattleStart(trainers,new IEnumerable<Fighter>[]{all[0],all[1]},x=>log+=x+"\r\n");for(int i=0;i<2;i++)EntryTriggers.Enter(Fighters[i],EntryReason.Lead,logLine=>log+=logLine+"\r\n");RuntimeTriggers.OpponentEntered(Fighters[0],Fighters[1],x=>log+=x+"\r\n");RuntimeTriggers.OpponentEntered(Fighters[1],Fighters[0],x=>log+=x+"\r\n");undo.Clear();RefreshView();}
 }
}
