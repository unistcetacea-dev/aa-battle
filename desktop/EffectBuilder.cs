...
namespace AABattle {
 ...
 partial class DataEditorForm {
  ...
  Control BuildInlineEffectBuilder(){
   var panel=new Panel{Dock=DockStyle.Top,Height=310,Padding=new Padding(2)};var table=Theme.Table();table.Dock=DockStyle.Fill;panel.Controls.Add(table);
   effectKind=Theme.Combo(new[]{"능력 랭크 변화","수치 배율","최대 HP의 1/N"},"능력 랭크 변화");effectKind.DropDownStyle=ComboBoxStyle.DropDown;effectKind.KeyPress+=(s,e)=>e.Handled=true;
   ...
   Theme.Row(table,"구조",effectKind);Theme.Row(table,"대상",effectTarget);Theme.Row(table,"종류",effectAction);Theme.Row(table,"배율 / N·랭크",Pair(effectRate,effectDenominator));Theme.Row(table,"빈사",effectCanFaint);Theme.Row(table,"결과",effectPreview);
   EventHandler refresh=(s,e)=>RefreshStructuredEffect();effectKind.SelectedIndexChanged+=(s,e)=>{RefreshEffectTargets();RefreshEffectActions();RefreshStructuredEffect();};effectTarget.TextChanged+=refresh;effectAction.SelectedIndexChanged+=refresh;effectRate.ValueChanged+=refresh;effectDenominator.ValueChanged+=refresh;effectCanFaint.CheckedChanged+=refresh;RefreshEffectTargets();RefreshEffectActions();RefreshStructuredEffect();return panel;
  }
