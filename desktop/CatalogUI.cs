...
namespace AABattle {
 ...
 partial class DataEditorForm {
  DataGridView moveDex,abilityDex;PixelSearchBox dexMoveSearch,abilitySearch;PixelText dexMoveDetail,abilityDetail,triggerPreview;ComboBox triggerKind,triggerSubject,triggerEntryMethod,triggerType,triggerAptitude,triggerRank,triggerActivation,triggerMoveKind,triggerMove,effectSubject;TextBox triggerGranted;FlatButton triggerStatusButton;string[] triggerStatusNames=new string[0];TableLayoutPanel triggerTable;
  const string MoveSubcategoryAll="전체 기술",MoveSubcategoryAttack="공격기술",MoveSubcategoryStatus="변화기술",MoveSubcategoryType="타입 지정",MoveSubcategoryName="기술 지정";
  static readonly string[] MoveSubcategories={MoveSubcategoryAll,MoveSubcategoryAttack,MoveSubcategoryStatus,MoveSubcategoryType,MoveSubcategoryName};
  ...
  Panel BuildStructuredTriggerPanel(){var panel=new Panel{Dock=DockStyle.Fill,Padding=new Padding(10)};var header=Theme.Label("트리거 구조 · 원문 없이 값으로 구성",40);var help=Theme.Label("구조를 고르면 그 조건에 필요한 값만 표시합니다.",58);help.ForeColor=Theme.Muted;triggerKind=Theme.Combo(new[]{"상시 (X)",EntryTriggers.Label,DefeatTriggers.Label,TriggerStructures.ObserverLabel,TriggerStructures.TurnEndLabel,TriggerStructures.FieldLabel,TriggerStructures.SwitchLabel,TriggerStructures.ReceivedLabel,TriggerStructures.LeaveLabel,"등장 방법 지정","적진 타입 포켓몬","트레이너 자질 등급",TriggerStructures.ReceivedOrNullifiedLabel},"상시 (X)");triggerMoveKind=Theme.Combo(MoveSubcategories,MoveSubcategoryAll);triggerEntryMethod=Theme.Combo(new[]{"선발","죽어내밀기","임의교대","선발 또는 죽어내밀기"},"선발");triggerType=Theme.Combo(catalogDb.types,"불꽃");triggerAptitude=Theme.Combo(new[]{"지시","육성","통솔","능력"},"통솔");triggerRank=Theme.Combo(new[]{"E","D","C","B","A","AA","AAA","S"},"A");triggerMove=Theme.Combo(catalogDb.moves.Select(x=>x.name).OrderBy(x=>x).ToArray(),catalogDb.moves.First().name);triggerGranted=new TextBox{Dock=DockStyle.Fill,Font=new Font("맑은 고딕",10),Text="계약의 특권"};triggerStatusButton=new FlatButton{Dock=DockStyle.Fill,Font=Theme.UI(8),Text="상태 선택 · 검색/토글"};triggerStatusButton.Click+=(s,e)=>PickTriggerStatuses();triggerPreview=new PixelText{Dock=DockStyle.Fill};triggerTable=Theme.Table();Theme.Row(triggerTable,"구조",triggerKind);Theme.Row(triggerTable,"기술 분류",triggerMoveKind);Theme.Row(triggerTable,"등장 방법",triggerEntryMethod);Theme.Row(triggerTable,"타입",triggerType);Theme.Row(triggerTable,"기술",triggerMove);Theme.Row(triggerTable,"부여된 이름",triggerGranted);Theme.Row(triggerTable,"상태 이름",triggerStatusButton);Theme.Row(triggerTable,"트레이너 자질",triggerAptitude);Theme.Row(triggerTable,"자질 등급",triggerRank);Theme.Row(triggerTable,"결과",triggerPreview);var apply=Button("구조 트리거 입력",220,()=>ApplyPart(true));apply.Dock=DockStyle.Bottom;panel.Controls.Add(triggerTable);panel.Controls.Add(help);panel.Controls.Add(header);panel.Controls.Add(apply);EventHandler refresh=(s,e)=>RefreshStructuredTrigger();triggerKind.SelectedIndexChanged+=refresh;triggerMoveKind.SelectedIndexChanged+=refresh;triggerEntryMethod.SelectedIndexChanged+=refresh;triggerType.SelectedIndexChanged+=refresh;triggerMove.SelectedIndexChanged+=refresh;triggerGranted.TextChanged+=refresh;triggerAptitude.SelectedIndexChanged+=refresh;triggerRank.SelectedIndexChanged+=refresh;RefreshStructuredTrigger();return panel;}
  ...
  void RefreshStructuredTrigger(){
   if(triggerPreview==null)return;
   if(triggerActivation==null){triggerActivation=Theme.Combo(new[]{"자동","선행"},"자동");Theme.Row(triggerTable,"입력 방식",triggerActivation);}
   if(triggerSubject==null){triggerSubject=Theme.Combo(new[]{"자신","아군","상대"},"자신");triggerSubject.SelectedIndexChanged+=(s,e)=>RefreshStructuredTrigger();Theme.Row(triggerTable,"주체",triggerSubject);}
   foreach(string item in new[]{TriggerStructures.HereLabel,TriggerStructures.SelfOrderLabel,TriggerStructures.AllyOrderLabel,TriggerStructures.AnalyzedLabel,TriggerStructures.BattleStartLabel,TriggerStructures.OpponentEntryLabel,TriggerStructures.MoveUseLabel,TriggerStructures.AttackSuccessLabel,TriggerStructures.DamagedLabel,"XX가 부여되어 있을 때","상태이상일 때","상태이상이 되었을 때","상태변화일 때","상태변화가 되었을 때"})if(!triggerKind.Items.Contains(item))triggerKind.Items.Add(item);
   string kind=triggerKind.Text;bool entry=kind=="등장 방법 지정",move=kind==TriggerStructures.MoveUseLabel,moveType=move&&triggerMoveKind.Text==MoveSubcategoryType,moveName=move&&triggerMoveKind.Text==MoveSubcategoryName,type=kind=="적진 타입 포켓몬"||moveType,status=IsStatusKind(kind),granted=kind=="XX가 부여되어 있을 때",aptitude=kind=="트레이너 자질 등급";
   if(status){bool change=kind.StartsWith("상태변화");var allowed=StateDex.StatusNames(change?"상태변화":"상태이상");triggerStatusNames=(triggerStatusNames??new string[0]).Where(x=>allowed.Contains(x)).ToArray();triggerStatusButton.Text=triggerStatusNames.Length==0?"상태 선택 · 검색/토글":("선택: "+string.Join(", ",triggerStatusNames));}else triggerStatusButton.Text="상태 선택 · 검색/토글";
   ShowTriggerRow(triggerMoveKind,move,"기술 분류");ShowTriggerRow(triggerEntryMethod,entry);ShowTriggerRow(triggerType,type);ShowTriggerRow(triggerMove,moveName);ShowTriggerRow(triggerGranted,granted,"부여된 이름");ShowTriggerRow(triggerStatusButton,status,"상태 이름");ShowTriggerRow(triggerAptitude,aptitude);ShowTriggerRow(triggerRank,aptitude);
   if(kind=="상시 (X)")triggerPreview.Text=TriggerStructures.AlwaysLabel;
   else if(entry)triggerPreview.Text=triggerEntryMethod.Text=="선발"?TriggerStructures.LeadEntryLabel:triggerEntryMethod.Text=="죽어내밀기"?TriggerStructures.ReplacementEntryLabel:triggerEntryMethod.Text=="임의교대"?TriggerStructures.VoluntaryEntryLabel:TriggerStructures.LeadOrReplacementEntryLabel;
   else if(kind=="적진 타입 포켓몬")triggerPreview.Text=TriggerStructures.OpponentType(triggerType.Text);
   else if(move){string sub=triggerMoveKind.Text;triggerPreview.Text=sub==MoveSubcategoryAttack?TriggerStructures.AttackMoveUseLabel:sub==MoveSubcategoryStatus?TriggerStructures.StatusMoveUseLabel:sub==MoveSubcategoryType?TriggerStructures.MoveType(triggerType.Text):sub==MoveSubcategoryName?TriggerStructures.MoveName(triggerMove.Text):TriggerStructures.MoveUseLabel;}
   else if(status)triggerPreview.Text=triggerStatusNames.Length==0?"상태를 검색·토글로 선택하세요.":string.Join("\r\n",triggerStatusNames.Select(x=>TriggerStructures.StatusCondition(triggerSubject.Text,x,kind.Contains("되었을 때"),kind.StartsWith("상태변화"))));
   else if(granted)triggerPreview.Text=TriggerStructures.Granted(triggerGranted.Text.Length>0?triggerGranted.Text:"이름 미지정");
   else if(aptitude)triggerPreview.Text=TriggerStructures.TrainerAptitude(triggerAptitude.Text,triggerRank.Text);
   else triggerPreview.Text=kind;
  }
  ...
  void LoadPotentialStructureSettings(){
   ...
   if(triggerMoveKind!=null)triggerMoveKind.SelectedItem=MoveSubcategoryAll;
   ...
   if(statusLines.Length>0){var first=statusLines[0];triggerSubject.SelectedItem=first.subject;triggerStatusNames=statusLines.Where(x=>x.subject==first.subject&&x.became==first.became&&x.change==first.change).Select(x=>x.name).Distinct().ToArray();string kind=first.change?(first.became?"상태변화가 되었을 때":"상태변화일 때"):(first.became?"상태이상이 되었을 때":"상태이상일 때");if(triggerKind.Items.Contains(kind))triggerKind.SelectedItem=kind;}
    ...
    foreach(string line in lines){if(TriggerStructures.TryMoveType(line,out moveType))break;if(TriggerStructures.TryMoveName(line,out moveName))break;}
    if(moveType.Length>0){triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryType;if(triggerType.Items.Contains(moveType))triggerType.SelectedItem=moveType;}
    else if(moveName.Length>0){triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryName;if(triggerMove.Items.Contains(moveName))triggerMove.SelectedItem=moveName;}
    else {string structured=lines.Select(TriggerStructures.Normalize).FirstOrDefault(x=>x==TriggerStructures.AttackMoveUseLabel||x==TriggerStructures.StatusMoveUseLabel||x==TriggerStructures.MoveUseLabel||triggerKind.Items.Contains(x));if(structured==TriggerStructures.AttackMoveUseLabel){triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryAttack;}else if(structured==TriggerStructures.StatusMoveUseLabel){triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryStatus;}else if(structured==TriggerStructures.MoveUseLabel){triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryAll;}else if(structured!=null)triggerKind.SelectedItem=structured;}
   }
  ...
  void LoadStructuredEffectSettings(string text){
   ...
   if(EffectStructures.IsSwitchRestriction(normalized)){effectKind.SelectedItem="교대 제한";effectTarget.SelectedItem="상대";effectAction.SelectedItem="통상교대 외 수단 제한";return;}
  ...
  public void TestCatalogUi(){
   TestRetroEditor();
   if(tabs.TabPages[4].Text!="기술 도감"||tabs.TabPages[6].Text!="특성 도감"||abilityDex==null||triggerKind==null||triggerMoveKind==null||triggerKind.Items.Count<31||!triggerKind.Items.Contains(TriggerStructures.MoveUseLabel)||triggerKind.Items.Contains(TriggerStructures.AttackMoveUseLabel)||triggerKind.Items.Contains(TriggerStructures.StatusMoveUseLabel)||!ConfirmedTriggerCatalog.Items.All(x=>triggerKind.Items.Contains(x))||effectKind==null||triggerSubject==null||effectSubject==null)throw new Exception("Structured trigger, subject and effect UI missing");
   ...
   if(!potentials.Columns.Contains("targetType")||potentials.Columns["targetType"].Visible||potentials.Columns.Contains("activation")||potentials.Columns.Contains("uses")||!potentials.Columns.Contains("disabled")||!(potentials.Columns["disabled"] is PixelDisableColumn))throw new Exception("Potential editor columns or pixel toggle invalid");
    ...
    triggerKind.SelectedItem=TriggerStructures.MoveUseLabel;triggerMoveKind.SelectedItem=MoveSubcategoryAll;if(!triggerMoveKind.Visible||triggerType.Visible||triggerMove.Visible||ConfiguredTrigger()!=TriggerStructures.MoveUseLabel)throw new Exception("Move trigger parent selection failed");triggerMoveKind.SelectedItem=MoveSubcategoryAttack;if(ConfiguredTrigger()!=TriggerStructures.AttackMoveUseLabel)throw new Exception("Attack move trigger subcategory failed");triggerMoveKind.SelectedItem=MoveSubcategoryStatus;if(ConfiguredTrigger()!=TriggerStructures.StatusMoveUseLabel)throw new Exception("Status move trigger subcategory failed");triggerMoveKind.SelectedItem=MoveSubcategoryType;triggerType.SelectedItem="불꽃";if(!triggerType.Visible||triggerMove.Visible||ConfiguredTrigger()!=TriggerStructures.MoveType("불꽃"))throw new Exception("Move type trigger subcategory failed");triggerMoveKind.SelectedItem=MoveSubcategoryName;triggerMove.SelectedItem="비바라기";if(!triggerMove.Visible||triggerType.Visible||ConfiguredTrigger()!=TriggerStructures.MoveName("비바라기"))throw new Exception("Move name trigger subcategory failed");
    ...
    string legacyTrigger=Cell(row,"trigger");row.Cells["trigger"].Value=TriggerStructures.AttackMoveUseLabel;LoadPotentialStructureSettings();if(triggerKind.Text!=TriggerStructures.MoveUseLabel||triggerMoveKind.Text!=MoveSubcategoryAttack||ConfiguredTrigger()!=TriggerStructures.AttackMoveUseLabel)throw new Exception("Legacy attack move trigger loading failed");row.Cells["trigger"].Value=legacyTrigger;LoadPotentialStructureSettings();
