...
namespace AABattle {
 ...
 partial class DataEditorForm {
  DataGridView moveDex;PixelSearchBox dexMoveSearch;PixelText dexMoveDetail,triggerPreview;ComboBox triggerKind,triggerEntryMethod,triggerType,triggerAptitude,triggerRank,triggerActivation,triggerMove;TextBox triggerGranted;TableLayoutPanel triggerTable;
  Label moveCount;
  ...
  void ShowTriggerRow(Control control,bool shown){if(triggerTable==null||control==null)return;int row=triggerTable.GetRow(control);triggerTable.RowStyles[row].Height=shown?42:0;control.Visible=shown;var label=triggerTable.GetControlFromPosition(0,row);if(label!=null)label.Visible=shown;}
  void RefreshStructuredTrigger(){if(triggerPreview==null)return;if(triggerActivation==null){triggerActivation=Theme.Combo(new[]{"자동","선행"},"자동");Theme.Row(triggerTable,"입력 방식",triggerActivation);}if(triggerKind.Items.Count==13)triggerKind.Items.AddRange(new object[]{TriggerStructures.HereLabel,TriggerStructures.SelfOrderLabel,TriggerStructures.AllyOrderLabel,TriggerStructures.AnalyzedLabel,TriggerStructures.BattleStartLabel,TriggerStructures.OpponentEntryLabel,TriggerStructures.MoveUseLabel,TriggerStructures.AttackMoveUseLabel,TriggerStructures.StatusMoveUseLabel,"타입 기술을 내보낼 때","특정 기술을 내보낼 때",TriggerStructures.AttackSuccessLabel,TriggerStructures.DamagedLabel,"XX가 부여되어 있을 때"});string kind=triggerKind.Text;bool entry=kind=="등장 방법 지정",type=kind=="적진 타입 포켓몬"||kind=="타입 기술을 내보낼 때",move=kind=="특정 기술을 내보낼 때",granted=kind=="XX가 부여되어 있을 때",aptitude=kind=="트레이너 자질 등급";ShowTriggerRow(triggerEntryMethod,entry);ShowTriggerRow(triggerType,type);ShowTriggerRow(triggerMove,move);ShowTriggerRow(triggerGranted,granted);ShowTriggerRow(triggerAptitude,aptitude);ShowTriggerRow(triggerRank,aptitude);if(kind=="상시 (X)")triggerPreview.Text=TriggerStructures.AlwaysLabel;else if(entry)triggerPreview.Text=triggerEntryMethod.Text=="선발"?TriggerStructures.LeadEntryLabel:triggerEntryMethod.Text=="죽어내밀기"?TriggerStructures.ReplacementEntryLabel:triggerEntryMethod.Text=="임의교대"?TriggerStructures.VoluntaryEntryLabel:TriggerStructures.LeadOrReplacementEntryLabel;else if(kind=="적진 타입 포켓몬")triggerPreview.Text=TriggerStructures.OpponentType(triggerType.Text);else if(kind=="타입 기술을 내보낼 때")triggerPreview.Text=TriggerStructures.MoveType(triggerType.Text);else if(move)triggerPreview.Text=TriggerStructures.MoveName(triggerMove.Text);else if(granted)triggerPreview.Text=TriggerStructures.Granted(triggerGranted.Text.Length>0?triggerGranted.Text:"이름 미지정");else if(aptitude)triggerPreview.Text=TriggerStructures.TrainerAptitude(triggerAptitude.Text,triggerRank.Text);else triggerPreview.Text=kind;}
  ...
  void ApplyPart(bool trigger){
   ...
   if(trigger){var record=row.Tag as PotentialRecord??new PotentialRecord();record.activation=triggerActivation==null?"자동":triggerActivation.Text;row.Tag=record;}
  ...
  public void TestCatalogUi(){
   if(tabs.TabPages[4].Text!="기술 도감"||tabs.TabPages[5].Text!="포텐셜 도감"||triggerKind==null||triggerKind.Items.Count!=27||effectKind==null)throw new Exception("Structured trigger and effect UI missing");
