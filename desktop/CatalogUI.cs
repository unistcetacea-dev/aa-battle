...
namespace AABattle {
 ...
 partial class DataEditorForm {
  ...
  void LoadPotentialStructureSettings(){if(loading)return;var row=SelectedPotentialRow();var record=row==null?null:row.Tag as PotentialRecord;if(record==null)return;loading=true;if(triggerActivation!=null)triggerActivation.SelectedItem=triggerActivation.Items.Contains(record.activation)?record.activation:"자동";if(triggerSubject!=null)triggerSubject.SelectedItem=triggerSubject.Items.Contains(record.triggerSubject)?record.triggerSubject:"자신";if(effectSubject!=null)effectSubject.SelectedItem=effectSubject.Items.Contains(record.effectSubject)?record.effectSubject:"자신";triggerStatusNames=new string[0];string[] lines=(Cell(row,"trigger")??"").Split(new[]{'\r','\n'},StringSplitOptions.RemoveEmptyEntries);string name;var statusLines=lines.Select(x=>{string s,n;bool b,c;return TriggerStructures.TryStatusCondition(x,out s,out n,out b,out c)?new{subject=s,name=n,became=b,change=c}:null;}).Where(x=>x!=null).ToArray();if(statusLines.Length>0){var first=statusLines[0];triggerSubject.SelectedItem=first.subject;triggerStatusNames=statusLines.Where(x=>x.subject==first.subject&&x.became==first.became&&x.change==first.change).Select(x=>x.name).Distinct().ToArray();string kind=first.change?(first.became?"상태변화가 되었을 때":"상태변화일 때"):(first.became?"상태이상이 되었을 때":"상태이상일 때");if(triggerKind.Items.Contains(kind))triggerKind.SelectedItem=kind;}else if(lines.Length>0&&TriggerStructures.TryGranted(lines[0],out name)){triggerGranted.Text=name;if(triggerKind.Items.Contains("XX가 부여되어 있을 때"))triggerKind.SelectedItem="XX가 부여되어 있을 때";}loading=false;RefreshStructuredTrigger();RefreshStructuredEffect();}
