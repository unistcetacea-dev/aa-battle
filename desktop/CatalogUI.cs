...
namespace AABattle {
 ...
 partial class DataEditorForm {
  DataGridView moveDex;PixelSearchBox dexMoveSearch;PixelText dexMoveDetail,triggerPreview;ComboBox triggerKind,triggerEntryMethod,triggerType,triggerAptitude,triggerRank;TableLayoutPanel triggerTable;
  Label moveCount;
  ...
  Panel BuildStructuredTriggerPanel(){var panel=new Panel{Dock=DockStyle.Fill,Padding=new Padding(10)};var header=Theme.Label("트리거 구조 · 원문 없이 값으로 구성",40);var help=Theme.Label("구조를 고르면 그 조건에 필요한 값만 표시합니다.",58);help.ForeColor=Theme.Muted;triggerKind=Theme.Combo(new[]{"상시 (X)",EntryTriggers.Label,DefeatTriggers.Label,TriggerStructures.ObserverLabel,TriggerStructures.TurnEndLabel,TriggerStructures.FieldLabel,TriggerStructures.SwitchLabel,TriggerStructures.ReceivedLabel,TriggerStructures.LeaveLabel,"등장 방법 지정","적진 타입 포켓몬","트레이너 자질 등급",TriggerStructures.ReceivedOrNullifiedLabel},"상시 (X)");triggerEntryMethod=Theme.Combo(new[]{"선발","죽어내밀기","임의교대","선발 또는 죽어내밀기"},"선발");triggerType=Theme.Combo(catalogDb.types,"불꽃");triggerAptitude=Theme.Combo(new[]{"지시","육성","통솔","능력"},"통솔");triggerRank=Theme.Combo(new[]{"E","D","C","B","A","AA","AAA","S"},"A");triggerPreview=new PixelText{Dock=DockStyle.Fill};triggerTable=Theme.Table();Theme.Row(triggerTable,"구조",triggerKind);Theme.Row(triggerTable,"등장 방법",triggerEntryMethod);Theme.Row(triggerTable,"상대 타입",triggerType);Theme.Row(triggerTable,"트레이너 자질",triggerAptitude);Theme.Row(triggerTable,"자질 등급",triggerRank);Theme.Row(triggerTable,"결과",triggerPreview);var apply=Button("구조 트리거 입력",220,()=>ApplyPart(true));apply.Dock=DockStyle.Bottom;panel.Controls.Add(triggerTable);panel.Controls.Add(help);panel.Controls.Add(header);panel.Controls.Add(apply);EventHandler refresh=(s,e)=>RefreshStructuredTrigger();triggerKind.SelectedIndexChanged+=refresh;triggerEntryMethod.SelectedIndexChanged+=refresh;triggerType.SelectedIndexChanged+=refresh;triggerAptitude.SelectedIndexChanged+=refresh;triggerRank.SelectedIndexChanged+=refresh;RefreshStructuredTrigger();return panel;}
  ...
  void RefreshStructuredTrigger(){if(triggerPreview==null)return;string kind=triggerKind.Text;bool entry=kind=="등장 방법 지정",type=kind=="적진 타입 포켓몬",aptitude=kind=="트레이너 자질 등급";ShowTriggerRow(triggerEntryMethod,entry);ShowTriggerRow(triggerType,type);ShowTriggerRow(triggerAptitude,aptitude);ShowTriggerRow(triggerRank,aptitude);if(kind=="상시 (X)")triggerPreview.Text=TriggerStructures.AlwaysLabel;else if(entry)triggerPreview.Text=triggerEntryMethod.Text=="선발"?TriggerStructures.LeadEntryLabel:triggerEntryMethod.Text=="죽어내밀기"?TriggerStructures.ReplacementEntryLabel:triggerEntryMethod.Text=="임의교대"?TriggerStructures.VoluntaryEntryLabel:TriggerStructures.LeadOrReplacementEntryLabel;else if(type)triggerPreview.Text=TriggerStructures.OpponentType(triggerType.Text);else if(aptitude)triggerPreview.Text=TriggerStructures.TrainerAptitude(triggerAptitude.Text,triggerRank.Text);else triggerPreview.Text=kind;}
  ...
  public void TestCatalogUi(){
   if(tabs.TabPages[4].Text!="기술 도감"||tabs.TabPages[5].Text!="포텐셜 도감"||triggerKind==null||triggerKind.Items.Count!=13||effectKind==null)throw new Exception("Structured trigger and effect UI missing");
