...
namespace AABattle {
 ...
 public static string Build(Database db,Fighter[] fighters,Rules rules,TurnSelection[] selections,TrainerRecord[] trainers,TrainerBattleState[] trainerStates,IEnumerable<Fighter>[] reserves=null,bool[] here=null){
   var text=new StringBuilder();Field(text,fighters,rules);
   for(int side=0;side<2;side++){text.AppendLine().Append("━━━━━━━━ ").Append(side+1).Append("팀 ━━━━━━━━\n");FighterBlock(text,db,fighters[side],fighters[1-side],rules,selections[side],side,trainers[side],here!=null&&side<here.Length&&here[side]);var observers=reserves==null?new string[0]:RuntimeTriggers.Observers(reserves[side]).ToArray();text.Append("대기 옵저버: ").AppendLine(observers.Length==0?"없음":string.Join(" / ",observers));TrainerBlock(text,trainers[side],trainerStates[side],selections[side]);}
   text.AppendLine().Append("━━━━━━━━ 설치물 ━━━━━━━━\n");for(int side=0;side<2;side++)text.Append(side+1).Append("팀 진영: ").AppendLine(Hazards(rules.Sides[side]));text.AppendLine("제거: 고속스핀=아군 / 안개제거=양쪽");return text.ToString().TrimEnd();
  }
  public static void Test(Database db){var a=new Fighter(db.pokemon.First());var b=new Fighter(db.pokemon.Skip(1).First());a.Status="독";a.Stages[1]=2;a.CriticalStage=1;a.Multipliers[1]=1.5;var r=new Rules{Weather="모래바람",WeatherTurns=3};string report=Build(db,new[]{a,b},r,new[]{new TurnSelection(),new TurnSelection()},new TrainerRecord[2],new[]{new TrainerBattleState(),new TrainerBattleState()});if(!report.Contains("공격+2")||!report.Contains("C+1")||!report.Contains("공격 ×1.5")||!report.Contains("독 HP -")||!report.Contains("날씨: 모래바람 3T")||!report.Contains("기술 적용 배율"))throw new Exception("Battle state report failed");}
  static void Field(StringBuilder text,Fighter[] f,Rules r){
   text.AppendLine("【전체 장소】");string effective=Mechanics.Weather(f[0],f[1],r);text.Append("날씨: ").Append(r.Weather);if(r.Weather!="없음")text.Append(" ").Append(r.WeatherTurns).Append("T");if(effective!=r.Weather)text.Append(" → 효과 무효");text.AppendLine();text.Append("필드: ").Append(r.Terrain);if(r.Terrain!="없음")text.Append(" ").Append(r.TerrainTurns).Append("T");text.AppendLine();text.Append("트릭룸: ").Append(r.TrickRoom>0?r.TrickRoom+"T":"없음").Append(" / 중력: ").Append(r.Gravity>0?r.Gravity+"T":"없음").AppendLine();text.Append("설치물 1팀: ").Append(Hazards(r.Sides[0])).AppendLine();text.Append("설치물 2팀: ").Append(Hazards(r.Sides[1])).AppendLine();
  }
  static void FighterBlock(StringBuilder text,Database db,Fighter a,Fighter b,Rules r,TurnSelection selected,int side,TrainerRecord trainer,bool here){
   ...
   var ranks=Enumerable.Range(1,5).Where(i=>a.Stages[i]!=0).Select(i=>StatNames[i]+Signed(a.Stages[i])).ToList();if(a.AccuracyStage!=0)ranks.Add("명중"+Signed(a.AccuracyStage));if(a.EvasionStage!=0)ranks.Add("회피"+Signed(a.EvasionStage));if(a.CriticalStage!=0)ranks.Add("C"+Signed(a.CriticalStage));text.Append("랭크: ").AppendLine(ranks.Count==0?"변화 없음":string.Join(" / ",ranks));
   ...
   text.Append("여기다!: ").AppendLine(here?"성립":"미성립");var active=RuntimeTriggers.Field(a,b,trainer,here).Select(x=>"『"+x.name+"』").ToArray();text.Append("상시 트리거 성립: ").AppendLine(active.Length==0?"없음":string.Join(" ",active));text.Append("등록 포텐셜(판정 대기): ");var potentials=(a.Data.potentials??new Potential[0]).Where(x=>x!=null).Select(x=>"『"+x.name+"』").ToArray();text.AppendLine(potentials.Length==0?"없음":string.Join(" ",potentials));
  ...
  static string Hazards(SideFieldState h){var list=new List<string>();if(h.StealthRock)list.Add("스텔스록");if(h.Spikes>0)list.Add("압정 "+h.Spikes+"겹");if(h.ToxicSpikes>0)list.Add("독압정 "+h.ToxicSpikes+"겹");if(h.StickyWeb)list.Add("끈적끈적네트");return list.Count==0?"없음":string.Join(" / ",list);}
