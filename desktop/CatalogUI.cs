...
namespace AABattle {
 ...
 partial class DataEditorForm {
  DataGridView moveDex;PixelSearchBox dexMoveSearch;PixelText dexMoveDetail,triggerPreview;ComboBox triggerKind,triggerSubject,triggerEntryMethod,triggerType,triggerAptitude,triggerRank,triggerActivation,triggerMove,effectSubject;TextBox triggerGranted;TableLayoutPanel triggerTable;
  Label moveCount;
  ...
  void RefreshStructuredTrigger(){if(triggerPreview==null)return;if(triggerActivation==null){triggerActivation=Theme.Combo(new[]{"자동","선행"},"자동");Theme.Row(triggerTable,"입력 방식",triggerActivation);}if(triggerSubject==null){triggerSubject=Theme.Combo(new[]{"자신","아군","상대"},"자신");triggerSubject.SelectedIndexChanged+=(s,e)=>RefreshStructuredTrigger();Theme.Row(triggerTable,"주체",triggerSubject);}if(triggerKind.Items.Count==13)triggerKind.Items.AddRange(new object[]{TriggerStructures.HereLabel,TriggerStructures.SelfOrderLabel,TriggerStructures.AllyOrderLabel,TriggerStructures.AnalyzedLabel,TriggerStructures.BattleStartLabel,TriggerStructures.OpponentEntryLabel,TriggerStructures.MoveUseLabel,TriggerStructures.AttackMoveUseLabel,TriggerStructures.StatusMoveUseLabel,"타입 기술을 내보낼 때","특정 기술을 내보낼 때",TriggerStructures.AttackSuccessLabel,TriggerStructures.DamagedLabel,"XX가 부여되어 있을 때","상태이상일 때","상태이상이 되었을 때","상태변화일 때","상태변화가 되었을 때"});string kind=triggerKind.Text;bool entry=kind=="등장 방법 지정",type=kind=="적진 타입 포켓몬"||kind=="타입 기술을 내보낼 때",move=kind=="특정 기술을 내보낼 때",status=kind.StartsWith("상태이상")||kind.StartsWith("상태변화"),granted=kind=="XX가 부여되어 있을 때",aptitude=kind=="트레이너 자질 등급";ShowTriggerRow(triggerEntryMethod,entry);ShowTriggerRow(triggerType,type);ShowTriggerRow(triggerMove,move);ShowTriggerRow(triggerGranted,granted||status,status?"상태 이름":"부여된 이름");ShowTriggerRow(triggerAptitude,aptitude);ShowTriggerRow(triggerRank,aptitude);if(kind=="상시 (X)")triggerPreview.Text=TriggerStructures.AlwaysLabel;else if(entry)triggerPreview.Text=triggerEntryMethod.Text=="선발"?TriggerStructures.LeadEntryLabel:triggerEntryMethod.Text=="죽어내밀기"?TriggerStructures.ReplacementEntryLabel:triggerEntryMethod.Text=="임의교대"?TriggerStructures.VoluntaryEntryLabel:TriggerStructures.LeadOrReplacementEntryLabel;else if(kind=="적진 타입 포켓몬")triggerPreview.Text=TriggerStructures.OpponentType(triggerType.Text);else if(kind=="타입 기술을 내보낼 때")triggerPreview.Text=TriggerStructures.MoveType(triggerType.Text);else if(move)triggerPreview.Text=TriggerStructures.MoveName(triggerMove.Text);else if(status)triggerPreview.Text=TriggerStructures.StatusCondition(triggerSubject.Text,triggerGranted.Text.Length>0?triggerGranted.Text:"상태 미지정",kind.Contains("되었을 때"),kind.StartsWith("상태변화"));else if(granted)triggerPreview.Text=TriggerStructures.Granted(triggerGranted.Text.Length>0?triggerGranted.Text:"이름 미지정");else if(aptitude)triggerPreview.Text=TriggerStructures.TrainerAptitude(triggerAptitude.Text,triggerRank.Text);else triggerPreview.Text=kind;}
  string ConfiguredTrigger(){return triggerPreview==null?"":triggerPreview.Text;}
  void LoadPotentialStructureSettings(){if(loading)return;var row=SelectedPotentialRow();var record=row==null?null:row.Tag as PotentialRecord;if(record==null)return;loading=true;if(triggerActivation!=null)triggerActivation.SelectedItem=triggerActivation.Items.Contains(record.activation)?record.activation:"자동";if(triggerSubject!=null)triggerSubject.SelectedItem=triggerSubject.Items.Contains(record.triggerSubject)?record.triggerSubject:"자신";if(effectSubject!=null)effectSubject.SelectedItem=effectSubject.Items.Contains(record.effectSubject)?record.effectSubject:"자신";loading=false;RefreshStructuredTrigger();RefreshStructuredEffect();}
  ...
  public void TestCatalogUi(){
   if(tabs.TabPages[4].Text!="기술 도감"||tabs.TabPages[5].Text!="포텐셜 도감"||triggerKind==null||triggerKind.Items.Count!=31||effectKind==null||triggerSubject==null||effectSubject==null)throw new Exception("Structured trigger, subject and effect UI missing");
