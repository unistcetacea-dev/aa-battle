...
namespace AABattle {
 ...
 public static class PotentialStatRules {
  ...
  public static string ChooseExtremeBase(Pokemon pokemon,bool highest,Func<double> random){if(pokemon==null||pokemon.@base==null||pokemon.@base.Length<6)throw new ArgumentException("Pokemon base stats are required","pokemon");return Random[ExtremeIndex(pokemon.@base.Skip(1).Take(5).ToArray(),highest,random)];}
  public static void Test(){if(Arbitrary.Length!=8||Arbitrary.Last()!="C"||Random.Length!=5||Random.Contains("명중")||ChooseRandom(()=>0)!="공격"||ChooseRandom(()=>.999)!="속도"||ExtremeIndex(new[]{100,200,200,50,70},true,()=>0)!=1||ExtremeIndex(new[]{100,200,200,50,50},true,()=>.999)!=2||ExtremeIndex(new[]{100,200,200,50,50},false,()=>.999)!=4||ChooseExtremeBase(new Pokemon{@base=new[]{1,100,200,200,50,70}},true,()=>.999)!="특공")throw new Exception("Potential stat selection rules failed");}
 }

 public static class SpeciesStatRanks {
  static readonly int[] Starts={1,40,60,90,110,140,170,200};
  public static int[] RaisedRange(int value,int amount=1){int tier=Tier(value),modifier=Modifier(value,tier);int next=Math.Min(Starts.Length-1,tier+Math.Max(1,amount));if(next==Starts.Length-1)return new[]{200,999};int start=Starts[next],end=Starts[next+1]-1;if(modifier<0)return new[]{start,start};if(modifier>0)return new[]{end-4,end};return new[]{start+1,end-5};}
  static int Tier(int value){for(int i=Starts.Length-1;i>=0;i--)if(value>=Starts[i])return i;return 0;}
  static int Modifier(int value,int tier){if(tier>=Starts.Length-1)return 0;int start=Starts[tier],end=Starts[tier+1]-1;if(value==start)return -1;if(value>=end-4)return 1;return 0;}
  public static void Test(){if(!RaisedRange(90).SequenceEqual(new[]{110,110})||!RaisedRange(100).SequenceEqual(new[]{111,134})||!RaisedRange(105).SequenceEqual(new[]{135,139})||!RaisedRange(135).SequenceEqual(new[]{165,169})||!RaisedRange(90,2).SequenceEqual(new[]{140,140})||!RaisedRange(105,2).SequenceEqual(new[]{165,169}))throw new Exception("Species stat rank ranges failed");}
 }
 ...
 public static class EffectStructures {
  ...
  public static void Test(){string target,action;int n;if(!TryFraction("자신의 체력을 1/4 회복한다",out target,out action,out n)||target!="자신"||action!="회복"||n!=4)throw new Exception("Fraction structure failed");var collapsed=Collapse(PotentialParts.Build(PotentialParts.StandardEntries()).effects);if(!collapsed.Any(x=>x.structureKind=="scalar")||!collapsed.Any(x=>x.structureKind=="fraction")||!collapsed.Any(x=>x.structureKind=="rank"))throw new Exception("Effect structure collapse failed");FractionalHpRule.Test();PotentialStatRules.Test();SpeciesStatRanks.Test();}
 }

 partial class DataEditorForm {
  static readonly string[] RankEffectTargets={"자신의 「공격」","자신의 「방어」","자신의 「특공」","자신의 「특방」","자신의 「속도」","자신의 「명중」","자신의 「회피」","자신의 「C」","자신의 임의의 능력","자신의 랜덤한 능력치","자신의 가장 높은 능력","자신의 가장 낮은 능력","자신의 전능력치","상대의 「공격」","상대의 「방어」","상대의 「특공」","상대의 「특방」","상대의 「속도」","상대의 「명중」","상대의 「회피」","상대의 「C」","상대의 임의의 능력","상대의 랜덤한 능력치","상대의 가장 높은 능력","상대의 가장 낮은 능력"};
  static readonly string[] SpeciesEffectTargets={"자신의 「체력」","자신의 「공격」","자신의 「방어」","자신의 「특공」","자신의 「특방」","자신의 「속도」"};
  ComboBox effectKind,effectAction,effectTarget;NumericUpDown effectRate,effectDenominator,effectSpeciesValue;CheckBox effectCanFaint;PixelText effectPreview;
  Panel BuildStructuredEffectPanel(){var panel=new Panel{Dock=DockStyle.Fill,Padding=new Padding(10)};var header=Theme.Label("효과 구조 · 원문 없이 값으로 구성",40);header.Dock=DockStyle.Top;var help=Theme.Label("구조를 고른 뒤 대상과 수치를 정합니다. 구조 종류는 구현 작업에 따라 계속 추가됩니다.",52);help.Dock=DockStyle.Top;help.ForeColor=Theme.Muted;var apply=Button("구조 효과 입력",220,()=>ApplyPart(false));apply.Dock=DockStyle.Bottom;var builder=BuildInlineEffectBuilder();builder.Dock=DockStyle.Top;panel.Controls.Add(builder);panel.Controls.Add(help);panel.Controls.Add(header);panel.Controls.Add(apply);return panel;}
  Control BuildInlineEffectBuilder(){
   var panel=new Panel{Dock=DockStyle.Top,Height=352,Padding=new Padding(2)};var table=Theme.Table();table.Dock=DockStyle.Fill;panel.Controls.Add(table);
   effectKind=Theme.Combo(new[]{"능력 랭크 변화","종족치 변경","수치 배율","최대 HP의 1/N"},"능력 랭크 변화");effectKind.DropDownStyle=ComboBoxStyle.DropDown;effectKind.KeyPress+=(s,e)=>e.Handled=true;
   effectTarget=Theme.Combo(RankEffectTargets,"자신의 「공격」");effectTarget.DropDownStyle=ComboBoxStyle.DropDown;
   effectAction=Theme.Combo(new[]{"상승","하락"},"상승");effectAction.DropDownStyle=ComboBoxStyle.DropDown;effectAction.KeyPress+=(s,e)=>e.Handled=true;effectRate=new NumericUpDown{DecimalPlaces=2,Minimum=.01M,Maximum=10M,Increment=.05M,Value=1.5M,Dock=DockStyle.Fill,Font=new Font("맑은 고딕",10),BorderStyle=BorderStyle.FixedSingle};effectDenominator=new NumericUpDown{Minimum=1,Maximum=999,Value=1,Dock=DockStyle.Fill,Font=new Font("맑은 고딕",10),BorderStyle=BorderStyle.FixedSingle};effectSpeciesValue=new NumericUpDown{Minimum=1,Maximum=999,Value=110,Dock=DockStyle.Fill,Font=new Font("맑은 고딕",10),BorderStyle=BorderStyle.FixedSingle};effectCanFaint=new CheckBox{Text="이 대미지로 빈사 가능",Checked=true,Dock=DockStyle.Fill,Font=Theme.UI(8)};effectPreview=new PixelText{Dock=DockStyle.Fill};
   Theme.Row(table,"구조",effectKind);Theme.Row(table,"대상",effectTarget);Theme.Row(table,"종류",effectAction);Theme.Row(table,"배율 / N·랭크",Pair(effectRate,effectDenominator));Theme.Row(table,"설정 종족치",effectSpeciesValue);Theme.Row(table,"빈사",effectCanFaint);Theme.Row(table,"결과",effectPreview);
   EventHandler refresh=(s,e)=>RefreshStructuredEffect();effectKind.SelectedIndexChanged+=(s,e)=>{RefreshEffectTargets();RefreshEffectActions();RefreshStructuredEffect();};effectTarget.TextChanged+=refresh;effectAction.SelectedIndexChanged+=refresh;effectRate.ValueChanged+=refresh;effectDenominator.ValueChanged+=refresh;effectSpeciesValue.ValueChanged+=refresh;effectCanFaint.CheckedChanged+=refresh;RefreshEffectTargets();RefreshEffectActions();RefreshStructuredEffect();return panel;
  }
  ...
  void RefreshEffectTargets(){if(effectKind==null||effectTarget==null)return;string[] choices=effectKind.Text=="종족치 변경"?SpeciesEffectTargets:effectKind.Text=="최대 HP의 1/N"?new[]{"자신","상대","아군 1체","아군 전체"}:effectKind.Text=="수치 배율"?RankEffectTargets.Concat(new[]{"자신의 기술의 위력","자신이 주는 대미지","자신이 받는 대미지","상대의 기술의 위력","상대가 주는 대미지","상대가 받는 대미지"}).ToArray():RankEffectTargets;string current=effectTarget.Text;if(effectTarget.Items.Cast<object>().Select(x=>x.ToString()).SequenceEqual(choices))return;effectTarget.Items.Clear();effectTarget.Items.AddRange(choices.Cast<object>().ToArray());effectTarget.SelectedItem=choices.Contains(current)?current:choices[0];}
  void RefreshEffectActions(){if(effectKind==null||effectAction==null)return;string[] choices=effectKind.Text=="종족치 변경"?new[]{"가장 높은 종족치 복사","가장 낮은 종족치 복사","1랭크 상승","2랭크 상승","직접 수치 지정"}:effectKind.Text=="수치 배율"?new[]{"강화","약화"}:effectKind.Text=="최대 HP의 1/N"?new[]{"회복","대미지"}:new[]{"상승","하락"};string current=effectAction.Text;if(effectAction.Items.Cast<object>().Select(x=>x.ToString()).SequenceEqual(choices))return;effectAction.Items.Clear();effectAction.Items.AddRange(choices.Cast<object>().ToArray());effectAction.SelectedItem=choices.Contains(current)?current:choices[0];}
  int CurrentSpeciesValue(){var p=Current as PokemonRecord;if(p==null)return 0;var s=p.stats;if(effectTarget.Text.Contains("체력"))return s.hp;if(effectTarget.Text.Contains("공격"))return s.attack;if(effectTarget.Text.Contains("방어"))return s.defense;if(effectTarget.Text.Contains("특공"))return s.spAttack;if(effectTarget.Text.Contains("특방"))return s.spDefense;if(effectTarget.Text.Contains("속도"))return s.speed;return 0;}
  int SpeciesRaiseAmount(){return effectAction.Text=="2랭크 상승"?2:effectAction.Text=="1랭크 상승"?1:0;}
  void RefreshSpeciesRange(){int min=1,max=999,amount=SpeciesRaiseAmount();if(amount>0&&CurrentSpeciesValue()>0){var range=SpeciesStatRanks.RaisedRange(CurrentSpeciesValue(),amount);min=range[0];max=range[1];}effectSpeciesValue.Maximum=max;effectSpeciesValue.Minimum=min;if(effectSpeciesValue.Value<min)effectSpeciesValue.Value=min;if(effectSpeciesValue.Value>max)effectSpeciesValue.Value=max;}
  void RefreshStructuredEffect(){if(effectPreview==null)return;bool scalar=effectKind.Text=="수치 배율",fraction=effectKind.Text=="최대 HP의 1/N",rank=effectKind.Text=="능력 랭크 변화",species=effectKind.Text=="종족치 변경";int raise=SpeciesRaiseAmount();effectRate.Enabled=scalar;effectDenominator.Enabled=fraction||rank;effectSpeciesValue.Enabled=species&&(raise>0||effectAction.Text=="직접 수치 지정");effectCanFaint.Enabled=fraction&&effectAction.Text=="대미지";if(species){RefreshSpeciesRange();string target=effectTarget.Text+" 종족치";if(effectAction.Text=="가장 높은 종족치 복사")effectPreview.Text=target+"를 가장 높은 종족치로 한다.";else if(effectAction.Text=="가장 낮은 종족치 복사")effectPreview.Text=target+"를 가장 낮은 종족치로 한다.";else if(raise>0&&CurrentSpeciesValue()<=0)effectPreview.Text="포켓몬과 대상 종족치를 먼저 설정하세요.";else if(raise>0)effectPreview.Text=target+"를 「"+(int)effectSpeciesValue.Value+"」로 하여 "+raise+"랭크 올린다.";else effectPreview.Text=target+"를 「"+(int)effectSpeciesValue.Value+"」로 한다.";}else if(scalar){effectPreview.Text=EffectNormalizer.Canonical(effectTarget.Text,effectAction.Text=="약화"?"약화":"강화",(double)effectRate.Value);}else if(fraction){string target=new[]{"자신","상대","아군 1체","아군 전체"}.Contains(effectTarget.Text)?effectTarget.Text:"자신";int n=(int)effectDenominator.Value;effectPreview.Text=effectAction.Text=="대미지"?target+"에게 최대 HP의 1/"+n+" 대미지를 준다."+(effectCanFaint.Checked?"":" 이 대미지로는 빈사로 할 수 없다."):target+"의 체력을 최대 HP의 1/"+n+" 회복한다.";}else if(rank){if(effectDenominator.Value>3)effectDenominator.Value=3;effectPreview.Text=EffectNormalizer.CanonicalRank(effectTarget.Text,(effectAction.Text=="하락"?-1:1)*(int)effectDenominator.Value);}else effectPreview.Text="";effectKind.Invalidate();effectAction.Invalidate();}
  string ConfiguredEffect(){return effectPreview==null?"":effectPreview.Text;}
 }
}
